...
from ..serializers import (ContestSerializer, ContestRetrieveSerializer,
...
class ContestAnnouncementListAPI(APIView):
    ...
    def get(self, request):
        ...
        if not contest_id:
            return self.error("无效的参数，需要比赛信息")
        data = ContestAnnouncement.objects \
                .select_related("created_by__userprofile") \
                .filter(contest_id=contest_id, visible=True)
...
class ContestAPI(APIView):
    def get(self, request):
        ...
        try:
            ...
            ).get(id=id, visible=True)
        except Contest.DoesNotExist:
            return self.error("该比赛不存在")
        data = ContestRetrieveSerializer(contest, context={'request': request}).data
...
class ContestRankAPI(APIView):
    def get_rank(self):
        if self.contest.rule_type == ContestRuleType.ACM:
            return ACMContestRank.objects.filter(contest=self.contest,
                ...
                select_related("user__userprofile").order_by("-accepted_number", "total_time")
        else:
    ...
    def get(self, request):
        ...
        page_qs["results"] = serializer(
            page_qs["results"],
            many=True,
            is_contest_admin=is_contest_admin,
            context={'request': request},
        ).data
