...
class MakeContestProblemPublicAPIView(APIView):
    ...
    def post(self, request):
        ...
        # Set display ID.
        problem._id = problem.pk
        problem.save()

        # The default collection is "其他" or the first collection.
        try:
            problem.collections.add(Collection.objects.filter(name='其他')[0])
        except IndexError:
            problem.collections.add(Collection.objects.first())

        problem.tags.set(tags)

        # Finally set mark contest problem is a public problem.
        Problem.objects.filter(pk=data['id']).update(is_public=True)
